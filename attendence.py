...
class Attendance:
    ...
    def __init__(self,root):
        ...
        img = img.resize((680, 170), Image.Resampling.LANCZOS)
        self.photoimg = ImageTk.PhotoImage(img)

        f_lbl = Label(self.root, image=self.photoimg)
        f_lbl.place(x=0, y=0, width=680, height=170)

                # Second Image
        img1 = Image.open(r"F:\\CSE (Study Material,Documents)\\Python\\Attendence Management\\Images\\smart-attendance.jpg")
        img1 = img1.resize((680, 170), Image.Resampling.LANCZOS)
        self.photoimg1 = ImageTk.PhotoImage(img1)

        f_lbl = Label(self.root, image=self.photoimg1)
        f_lbl.place(x=681, y=0, width=680, height=170)
        ...
        main_frame.place(x=10, y=55, width=1350, height=500)
        ...
        #Student id
        studentId_label = Label(left_frame,text="Enrollment ID:",font=("verdana",12,"bold"),fg="navyblue",bg="white")
        ...
        btn_frame.place(x=0,y=410,width=656,height=50)
        ...
        search_frame.place(x=5, y=5, width=645, height=50)
        search_label = Label(search_frame, text="Search By:", font=("times now roman", 11, "bold"), bg="white")
        search_label.grid(row=0, column=0, padx=10,pady=11, sticky=W)
        ...
        table_frame.place(x=5,y=60,width=645,height=350)
    ...
    #Exprot button
        export_btn=Button(right_frame,command=self.exportCSVfromDatabase,text="Export CSV",width=12,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        export_btn.grid(row=0,column=1,padx=6,pady=8,sticky=W)
    # Update button
        update1_btn=Button(right_frame,command=self.update_data,text="Update",width=12,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        update1_btn.grid(row=0,column=2,padx=6,pady=420,sticky=W)
    #delete button
        del_btn=Button(right_frame,command=self.delete_data,text="Delete",width=12,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        del_btn.grid(row=0,column=3,padx=6,pady=410,sticky=W)
    ...
    def update_data(self):
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                ...
                if Update > 0:
                    conn = mysql.connector.connect(username='root', password='Kuldeep@16',host='localhost',database='face_recognition')
    ...
    def exportCSVfromDatabase(self):
        sql = "SELECT * FROM face_recognition.stdattendance;"
        db = mysql.connector.connect(host="localhost", database="face_recognition", user="root", passwd="Kuldeep@16", use_pure=True)
